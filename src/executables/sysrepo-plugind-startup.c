...
#if 1 /* Source code from sysrepo-plugind.c */
...
#endif /* Source code from sysrepo-plugind.c */
...
{
    return load_plugins(plugins, plugin_count);
}

static int srpd_cleanup(const char *pidfile, int pidfd, sr_conn_ctx_t *conn,
...
int srpd_parse_options(int argc, char **argv, srpd_opts_t *opts)
{
    ...
    int r, opt, rc = EXIT_FAILURE, debug = 0, fatal_fail = 0;
    ...
    opts->fatal_fail = fatal_fail;

    return EXIT_SUCCESS;
cleanup:
    return srpd_cleanup(pidfile, -1, NULL, NULL, 0, rc);
}
                 ...
                 bool send_sd_ready, bool occupy_this_thread)
{
    ...
    /* get options which from command line */
    debug = opts->debug;
    pidfile = opts->pidfile;
    log_level = opts->log_level;
    fatal_fail = opts->fatal_fail;
    ...
    srpd_teardown(true, sess, false,
                  /* meaningless for cleanup params here */
...
cleanup:
    return srpd_cleanup(pidfile, pidfd, conn, plugins, plugin_count, rc);
}
                   ...
                   struct srpd_plugin_s *plugins, int plugin_count, int rc)
{
    ...
    if (cleanup) {
        (void)srpd_cleanup(pidfile, pidfd, conn, plugins, plugin_count, rc);
    }
}
